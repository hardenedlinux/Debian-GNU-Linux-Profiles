...
​       这次实践中，我们将使用Suricata/Snort(视网络环境和资源，也可以相互补充)，以快速识别已知特征的攻击。Zeek同时结合Bro以维护重要数据。这些数据不仅提供对Suricata/Snort产生的告警进行适当分类的上下文，还提供不仅包括事件本身，也包括整个威胁的全面时间表：发生了什么，导致了该事件，威胁产生之后会发生什么。
